...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateTransformInputs() {
        ...
        if(sVal) sVal.textContent = parseFloat(composition.scale).toFixed(3);
    ...
    function drawComposition() {
        ...
        // Uso de imageSmoothingQuality en la vista previa
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        if (isDragging) { composition.x += dx; composition.y += dy; } 
            ...
            const newScale = Math.max(0.005, composition.scale + factor);
    ...
    // --- IMPLEMENTACIÓN DE STEPPED SCALING (Bicubic Sharper Emulation) ---
    // Reduce la imagen en pasos del 50% para mantener calidad y luego aplica enfoque
    function steppedResize(img, targetW, targetH) {
        ...
        while (curW * 0.5 > targetW) {
            ...
            tempCtx.drawImage(canvas, 0, 0, canvas.width, canvas.height, 0, 0, curW, curH);
        ...
        finalCtx.drawImage(canvas, 0, 0, curW, curH, 0, 0, targetW, targetH);

        // --- LÓGICA DINÁMICA ---
        // Cuanto más chico es el lienzo (menos píxeles de ancho),
        // más "fuerte" debe ser el efecto sharpen para evitar el blur.
        
        let dynamicStrength = 0.25; // Base normal (para imágenes grandes)

        if (targetW < 80) {
            // Lienzos muy pequeños (ej. 20cm) -> Enfoque muy agresivo
            dynamicStrength = 0.60; 
        } else if (targetW < 130) {
            // Lienzos medianos-chicos (ej. 30cm) -> Enfoque fuerte
            dynamicStrength = 0.40;
        }
        
        // Aplicamos la fuerza calculada
        applySharpenFilter(finalCtx, targetW, targetH, dynamicStrength); 

        return finalCanvas;
    }

    function applySharpenFilter(ctx, width, height, strength) {
        const imageData = ctx.getImageData(0, 0, width, height);
        applySharpen(imageData, strength); // Reutilizamos la lógica existente de sharpen
    ...
    const processImageClientSide = async () => {
        ...
        try {
            ...
            // Filtros CSS generales (brillo, contraste, etc)
            ...
            // --- NUEVA LÓGICA DE RESIZING ---
            // Calcular el tamaño visual final de la imagen
            const targetImageW = originalImageObject.width * composition.scale;
            const targetImageH = originalImageObject.height * composition.scale;

            // Determinar si usar reescalado avanzado (bicubic sharper emulation)
            let sourceDrawable;
            let drawScale;
            let drawW, drawH;

            // Si reducimos mucho la imagen (menos del 50%), usamos stepped resize
            // Esto emula "Bicubic Sharper"
            if (composition.scale < 0.5) {
                // Generar canvas temporal pre-escalado y enfocado
                sourceDrawable = steppedResize(originalImageObject, targetImageW, targetImageH);
                drawScale = 1; // Ya está escalado
                drawW = targetImageW;
                drawH = targetImageH;
            } else {
                // Si la escala es grande, usar default del navegador
                sourceDrawable = originalImageObject;
                drawScale = composition.scale;
                drawW = originalImageObject.width;
                drawH = originalImageObject.height;
                tempCtx.imageSmoothingEnabled = true;
                tempCtx.imageSmoothingQuality = 'high';
            }

            // Aplicar transformaciones al contexto principal
            const imgCenterX = composition.x + targetImageW / 2;
            const imgCenterY = composition.y + targetImageH / 2;
            
            tempCtx.translate(imgCenterX, imgCenterY);
            tempCtx.rotate(composition.rotation * Math.PI / 180);
            
            // Usar la escala adecuada (1 si ya se redimensionó, o la escala de composición si es la imagen original)
            tempCtx.scale(drawScale, drawScale);
            
            // Dibujar la fuente (Original o Pre-procesada)
            tempCtx.drawImage(sourceDrawable, -drawW / 2, -drawH / 2, drawW, drawH);
            ...
            // Post-Procesamiento Píxeles (Opacidad estricta)
            ...
            // Filtros JS de Pixel Art
            if (controls.blur_sharpen < 0) {
                // Extra sharpen seleccionado por slider, adicional al proceso bicubic
    ...
    highlightChecks.forEach(check => {
        check.addEventListener('change', (e) => {
            ...
            highlightChecks.forEach(c => c.checked = val); 
            updateHighlight();
